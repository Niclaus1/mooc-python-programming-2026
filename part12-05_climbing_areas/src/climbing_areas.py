...
def by_length(route: ClimbingArea):
    return route.routes()


def sort_by_number_of_routes(route: list):
    return sorted(route, key=by_length)


def by_difficulty(route: ClimbingArea):
    return route.hardest_route().grade


def sort_by_most_difficult(route: list):
    return sorted(route, key=by_difficulty, reverse=True)
